...
?>
    ...
    <div class="row container p-3 mt-5  shadow">    
        <?php
            $items = allData("SELECT * FROM product , fav WHERE product.productid = fav.productid AND fav.userid = $authid;");
            $count = 0;
            $counter = findData("SELECT count(product.productid) AS aa FROM product , fav WHERE product.productid = fav.productid AND fav.userid = $authid;");
           
            $datta = $counter['aa'];
            ?>
            <h6>ALL FAVORITE :: <b><?=$datta;?></b></h6>
            <?php
    ...
    <div class="col-lg-3 col-md-4 col-sm-6 my-2">
      ...
      <path d="M4 10.781c.148 1.667 1.513 2.85 3.591 3.003V15h1.043v-1.216c2.27-.179 3.678-1.438 3.678-3.3 0-1.59-.947-2.51-2.956-3.028l-.722-.187V3.467c1.122.11 1.879.714 2.07 1.616h1.47c-.166-1.6-1.54-2.748-3.54-2.875V1H7.591v1.233c-1.939.23-3.27 1.472-3.27 3.156 0 1.454.966 2.483 2.661 2.917l.61.162v4.031c-1.149-.17-1.94-.8-2.131-1.718H4zm3.391-3.836c-1.043-.263-1.6-.825-1.6-1.616 0-.944.704-1.641 1.8-1.828v3.495l-.2-.05zm1.591 1.872c1.287.323 1.852.859 1.852 1.769 0 1.097-.826 1.828-2.2 1.939V8.73l.348.086z"/>
                ...
                if (checkalreadyexist($k)) {
                       ?>
                       <a href="favorite?removefave=<?=$key['productid'];?>" class="btn " type="submit"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="white"  class="bi bi-heart-fill" viewBox="0 0 16 16">
                <path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z"/>
                </svg></a>
              <?php  
              }else   {?>     <a href="favorite?addfave=<?=$key['productid'];?>" class="btn " type="submit"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="white"  class="bi bi-heart" viewBox="0 0 16 16">
      ...
      <path d="M0 1.5A.5.5 0 0 1 .5 1H2a.5.5 0 0 1 .485.379L2.89 3H14.5a.5.5 0 0 1 .491.592l-1.5 8A.5.5 0 0 1 13 12H4a.5.5 0 0 1-.491-.408L2.01 3.607 1.61 2H.5a.5.5 0 0 1-.5-.5zM3.102 4l1.313 7h8.17l1.313-7H3.102zM5 12a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm7 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm-7 1a1 1 0 1 1 0 2 1 1 0 0 1 0-2zm7 0a1 1 0 1 1 0 2 1 1 0 0 1 0-2z"/>
                ...
                <a href="favorite?removecart=<?=$key['productid'];?>" class="btn" type="submit">
      ...
      <path d="M0 8s3-5.5 8-5.5S16 8 16 8s-3 5.5-8 5.5S0 8 0 8zm8 3.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7z"/>
                ...
                <small class="text-muted"><?=time_ago($key['updaterecord']);?></small>
              </div>
            </div>
    </div>
