...
<?php require_once('header.php');?>
    ...
    <div class="row container p-3 mt-5  shadow">    
        <?php
            $items = allData("SELECT * FROM `product` WHERE `category_id` = 2 ORDER BY `productupdate` DESC;");
            $count = 0;
            $counter = findData("SELECT count(`productid`) AS aa FROM `product` WHERE `category_id` = 2 ;  ");
